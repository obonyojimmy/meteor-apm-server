...
var component = FlowComponents.define('debug.timeline', function(props) {
  ...
  this.autorun(function() {
    ...
    if (sessionId && this.debugStoreInstance) {
      if (traceKey) {
        ...
        this.debugStoreInstance.getTrace(browserId, clientId, type, id, function(err, trace) {
          if (trace) {
  ...
  this.onRendered(function() {
    if (sessionId) {
      ...
      React.render(
        <TimelineComponent.Timeline sessionId={sessionId} debugStore={this.debugStoreInstance} />,
        dom
      );
    }
  });
