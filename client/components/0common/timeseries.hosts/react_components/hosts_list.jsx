    ...
    return (
      <tr>
        <td colSpan="2">
          ...
          <span className="glyphicon glyphicon-stop" style={{ color: this.props.color }} />
    ...
    return (
      <div>
        <div className="panel panel-default panel-hosts">
          <p className="selected-point">{this.getActivePointDate()}</p>
          <table className="table table-condensed table-active-data">
            ...
            <tbody>{this.getRowByHost(activeSeries)}</tbody>
          </table>
          <div className="div-table-content">
            <table className="table table-condensed">
              <tbody>{this.data.hostsArray.map(this.getRowByHost)}</tbody>
            </table>
          </div>
        </div>
      </div>
